...
%   s       -   tire slip vector            %
%   x_T_dot -   tire state derivatives      %
%               x_e_dot, y_e_dot, psi_e_dot %
...
%% Preparation
v_x = v_0QQ(1); v_y = v_0QQ(2); v_z = v_0QQ(3); % Contact velocities
omega_y = omega_0WW(2); omega_z = omega_0WW(3); % Angular velocities
x_e = x_T(1); y_e = x_T(2); psi_e = x_T(3);     % Tire deflections
...
L   = 2*sqrt(r_0*dz);   % Contact patch length
...
elseif F_z > tir.F_zMax
    F_z = tir.F_zMax;
    warning([num2str(t) 's: Tire normal load too high (' num2str(F_zUnlim) 'N).']);
end
...
if dF_0 < 2*F_M/s_M
    dF_0 = 2*F_M/s_M;
    warning([num2str(t) 's: Tire slip stiffness value dF_0 too low.'],'backtrace','off');
end
...
if s_G > s_S
    ...
    % Transition from adhesion to sliding
        ...
        if s_star < s_S
            ...
            warning([num2str(t) 's: Tire full sliding slip value s_S too low.'],'backtrace','off');
